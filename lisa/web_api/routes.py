...
def task_file_create():
    ...
    # get script architecture
    script_arch = None
    if 'script_arch' in request.form:
        try:
            script_arch = int(request.form['script_arch'])
        except ValueError:
            res = ErrorAPIResponse(2022).to_dict()
            return jsonify(res), 400
    ...
    # run analysis
    args = (file_path,)
    kwargs = {'pretty': pretty, 'exec_time': exec_time, 'script_arch':script_arch}
